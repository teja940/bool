...
export default function Coupes()
{
  ...
  let [barName, setBarName] = useState('Coupes')
  ...
  let [coupesRatingLimit, setCoupesRatingLimit] = useState(0)
  ...
  async function updateItsLit(itsLit)
  {
    ...
    firebase.database().ref('users/'+USERNAME).update({
      coupesRatingLimit: coupesRatingLimit+1
    })
  }
  ...
  async function itsLitListener()
  {
    ...
    firebase.database().ref('users/'+USERNAME).on('value', (snapshot)=>{
      var highScore1 = snapshot.val().coupesRatingLimit
      setCoupesRatingLimit(highScore1)
    })
  }
