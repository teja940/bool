...
export default function Hole()
{
  ...
  let [barName, setBarName] = useState('Hole')
  ...
  let [holeRatingLimit, setHoleRatingLimit] = useState(0)
  ...
  async function updateItsLit(itsLit)
  {
    ...
    firebase.database().ref('users/'+USERNAME).update({
      holeRatingLimit: holeRatingLimit+1
    })
  }
  ...
  async function itsLitListener()
  {
    ...
    firebase.database().ref('users/'+USERNAME).on('value', (snapshot)=>{
      var highScore1 = snapshot.val().holeRatingLimit
      setHoleRatingLimit(highScore1)
    })
  }
