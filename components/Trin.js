...
export default function Trin()
{
  ...
  let [barName, setBarName] = useState('Trin')
  ...
  let [trinRatingLimit, setTrinRatingLimit] = useState(0)
  ...
  async function updateItsLit(itsLit)
  {
    ...
    firebase.database().ref('users/'+USERNAME).update({
      trinRatingLimit: trinRatingLimit+1
    })
  }
  ...
  async function itsLitListener()
  {
    ...
    firebase.database().ref('users/'+USERNAME).on('value', (snapshot)=>{
      var highScore1 = snapshot.val().trinRatingLimit
      setTrinRatingLimit(highScore1)
    })
  }
