...
import { Text, View, ImageBackground, Image, StyleSheet, TouchableOpacity, TextPropTypes, Button} from 'react-native';
...
export default function Boylan()
{
  ...
  let [boylanRatingLimit, setBoylanRatingLimit] = useState(0)
  ...
  async function updateItsLit(itsLit)
  {
    ...
    firebase.database().ref('users/'+USERNAME).update({
      boylanRatingLimit: boylanRatingLimit+1
    })
  }
  ...
  async function itsLitListener()
  {
    ...
    firebase.database().ref('users/'+USERNAME).on('value', (snapshot)=>{
      var highScore1 = snapshot.val().boylanRatingLimit
      setBoylanRatingLimit(highScore1)
    })
  }
